...
const Breadcrumbs = () => {
  ...
  const pathnames = location.pathname.split("/").filter((x) => x); // Memecah pathname menjadi bagian-bagian
    ...
    <nav className="flex" aria-label="Breadcrumb">
      <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        {pathnames.map((name, index) => {
          // Buat URL untuk setiap segmen path
          ...
          return (
            <li key={name} aria-current={isLast ? "page" : undefined}>
              <div className="flex items-center">
                ...
                {isLast ? (
                  // Item terakhir (current page), tidak bisa diklik
