...
const AddJenisModal = ({ isOpen, onClose, onSave, initialData }) => {
  const [namaAsetSatu, setNamaAsetSatu] = useState("");
  const [namaAsetDua, setNamaAsetDua] = useState("");
  const [kodeAsetTiga, setKodeAsetTiga] = useState("");
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setKodeAsetTiga(initialData.kodeAset3 || "");
      setNamaAsetTiga(initialData.namaAset3 || "");
  ...
  const handleSubmit = (e) => {
    ...
    if (!namaAsetSatu || !namaAsetDua || !kodeAsetTiga || !namaAsetTiga) {
    ...
    const dataToSave = {
      ...
      aset1: namaAsetSatu,
      aset2: namaAsetDua,
      kodeAset3: kodeAsetTiga,
      namaAset3: namaAsetTiga,
      kode: kode,
    };
      ...
      <div className="relative w-full max-w-lg p-6 bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-800">
            {initialData ? "EDIT ASET 3" : "TAMBAH ASET 3"}
          </h2>
          <button
            onClick={onClose}
        ...
        <form className="max-h-[calc(100vh-180px)] overflow-y-auto pr-2 pb-4">
          ...
          <div className="mb-4">
            <label htmlFor="kodeAsetTiga" className="block mb-2 text-gray-700">
              Kode Aset 3: <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              id="kodeAsetTiga"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={kodeAsetTiga}
              onChange={(e) => setKodeAsetTiga(e.target.value)}
              required
              min="0"
            />
          </div>
