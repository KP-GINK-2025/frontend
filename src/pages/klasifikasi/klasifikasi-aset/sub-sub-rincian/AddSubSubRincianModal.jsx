...
const AddSubSubRincianModal = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const [kodeAsetTujuh, setKodeAsetTujuh] = useState("");
  const [namaAsetTujuh, setNamaAsetTujuh] = useState("");
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setNamaAsetEnam(initialData.aset6 || "");
      setKodeAsetTujuh(initialData.kodeAset7 || "");
      setNamaAsetTujuh(initialData.namaAset7 || "");
  ...
  const handleSubmit = (e) => {
    ...
    const dataToSave = {
      ...
      aset5: namaAsetLima,
      aset6: namaAsetEnam,
      kodeAset7: kodeAsetTujuh,
      namaAset7: namaAsetTujuh,
      kode: kode,
    };
        ...
        <form className="max-h-[calc(100vh-180px)] overflow-y-auto pr-2 pb-4">
          ...
          <div className="mb-4">
            ...
            >
              ...
              <option value="Tanah Bangunan Perumahan/G.Tempat Tinggal">
                Tanah Bangunan Perumahan/G.Tempat Tinggal
              </option>
            </select>
          </div>

          <div className="mb-4">
            <label htmlFor="kodeAsetTujuh" className="block mb-2 text-gray-700">
              Kode Aset 7: <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              id="kodeAsetTujuh"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={kodeAsetTujuh}
              onChange={(e) => setKodeAsetTujuh(e.target.value)}
              required
              min="0"
            />
          </div>
          ...
          <div className="mb-4">
            <label htmlFor="namaAsetTujuh" className="block mb-2 text-gray-700">
              Nama Aset 7: <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="namaAsetTujuh"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={namaAsetTujuh}
              onChange={(e) => setNamaAsetTujuh(e.target.value)}
              required
            />
          </div>
