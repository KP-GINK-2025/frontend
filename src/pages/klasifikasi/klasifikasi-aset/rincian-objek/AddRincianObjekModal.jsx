...
const AddRincianObjekModal = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const [kodeAsetLima, setKodeAsetLima] = useState("");
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setKodeAsetLima(initialData.kodeAset5 || "");
      setNamaAsetLima(initialData.namaAset5 || "");
  ...
  const handleSubmit = (e) => {
    ...
    const dataToSave = {
      ...
      aset3: namaAsetTiga,
      aset4: namaAsetEmpat,
      kodeAset5: kodeAsetLima,
      namaAset5: namaAsetLima,
      kode: kode,
    };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-full max-w-lg p-6 bg-white rounded-lg shadow-lg">
        ...
        <form className="max-h-[calc(100vh-180px)] overflow-y-auto pr-2 pb-4">
          ...
          <div className="mb-4">
            <label htmlFor="kodeAsetLima" className="block mb-2 text-gray-700">
              Kode Aset 5: <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              id="kodeAsetLima"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={kodeAsetLima}
              onChange={(e) => setKodeAsetLima(e.target.value)}
              required
              min="0"
            />
          </div>
...
export default AddRincianObjekModal;
